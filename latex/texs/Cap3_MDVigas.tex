...
\section{Energía de deformación}

Sea el pórtico mostrado en la \autoref{fig:porticoUdef} formado por barras de hormigón con $E$=30 GPa sometida a una carga puntual en el nodo $B$ de valor $P$= 15 kN y las longitudes que se muestran. Los pilares y la viga tienen sección cuadrada de 0.40 m de lado.
...
Las incógnitas para resolver la estructura por el Método de Slope Deflection son las siguientes.

\begin{equation}
	\theta_B \qquad \theta_C \qquad \Delta_{BC}
\end{equation}

Las condiciones a imponer para resolver la estructura son:
...
Planteando las ecuaciones del método y resolviendo el sistema de ecuaciones, se tiene,

\begin{equation}
	\theta_B = -4.69\cdot 10^{-4} \qquad \theta_C = -4.69\cdot 10^{-4} \qquad \Delta_{BC} = -1.13\cdot 10^{-2}
\end{equation}
...
Interesa calcular la energía de deformación interna de la estructura a flexión y directa y compararlas. La energía de deformación interna $U_{int}$ como ha sido visto, se calcula de acuerdo con la siguiente expresión,
